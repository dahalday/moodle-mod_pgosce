 ...
 * @return string
 */
function pgosce_render_select_input($name, $value, $label, array $options, $datafield = '') {
    $id = html_writer::random_id('pgosce_');
    $attrs = [
        'id' => $id,
    ...
    return html_writer::div(
        html_writer::tag('label', $label, ['for' => $id]) .
        html_writer::select($options, $name, (string)$value, false, $attrs),
        'form-group'
    );
}
...
function pgosce_render_criterion_editor(array $criterion, $sectionindex, $questionindex, $criterionindex) {
    ...
    $output .= html_writer::div(
        ...
        pgosce_render_select_input($prefix . '[markinputtype]', $markinputtype,
            get_string('criterionmarkinputtype', 'pgosce'), [
                'global' => get_string('markinputtypeglobal', 'pgosce'),
                'buttons' => get_string('markinputtypebuttons', 'pgosce'),
                'textbox' => get_string('markinputtypetextbox', 'pgosce'),
            ], 'criterion-markinputtype') .
        pgosce_render_select_input($prefix . '[markbuttonstep]', $markbuttonstep,
            get_string('criterionmarkbuttonstep', 'pgosce'), [
                '0' => get_string('markbuttonstepglobal', 'pgosce'),
            ...
            ], 'criterion-markbuttonstep'),
        'col-md-3 pgosce-criterion-mark-settings'
    );
    $output .= html_writer::div(
...
(function() {
        ...
        for (var s = 0; s < sections.length; s++) {
            ...
            for (var q = 0; q < questions.length; q++) {
                ...
                for (var c = 0; c < criteria.length; c++) {
                    ...
                    criteria[c].querySelector('[data-field=\"criterion-markinputtype\"]').name = 'rubric[' + s + '][questions][' + q + '][criteria][' + c + '][markinputtype]';
                    criteria[c].querySelector('[data-field=\"criterion-markbuttonstep\"]').name = 'rubric[' + s + '][questions][' + q + '][criteria][' + c + '][markbuttonstep]';
    ...
    function criterionHtml() {
        return '<div class=\"pgosce-criterion border rounded p-2 mb-2\">' +
            '<div class=\"row\">' +
            '<div class=\"col-md-8\"><div class=\"form-group\"><label>" . addslashes_js(get_string('criteria', 'pgosce')) . "</label><textarea data-field=\"criterion-description\" class=\"form-control\" rows=\"2\"></textarea></div></div>' +
            '<div class=\"col-md-3 pgosce-criterion-mark-settings\"><div class=\"form-group\"><label>" . addslashes_js(get_string('maxmark', 'pgosce')) . "</label><input data-field=\"criterion-maxmark\" type=\"number\" step=\"0.01\" min=\"0\" value=\"1\" class=\"form-control\"></div>' +
            '<div class=\"form-group\"><label>" . addslashes_js(get_string('criterionmarkinputtype', 'pgosce')) . "</label><select data-field=\"criterion-markinputtype\" class=\"custom-select form-control\"><option value=\"global\">" . addslashes_js(get_string('markinputtypeglobal', 'pgosce')) . "</option><option value=\"buttons\">" . addslashes_js(get_string('markinputtypebuttons', 'pgosce')) . "</option><option value=\"textbox\">" . addslashes_js(get_string('markinputtypetextbox', 'pgosce')) . "</option></select></div>' +
            '<div class=\"form-group\"><label>" . addslashes_js(get_string('criterionmarkbuttonstep', 'pgosce')) . "</label><select data-field=\"criterion-markbuttonstep\" class=\"custom-select form-control\"><option value=\"0\">" . addslashes_js(get_string('markbuttonstepglobal', 'pgosce')) . "</option><option value=\"1\">" . addslashes_js(get_string('markbuttonstepone', 'pgosce')) . "</option><option value=\"0.5\">" . addslashes_js(get_string('markbuttonstephalf', 'pgosce')) . "</option><option value=\"0.25\">" . addslashes_js(get_string('markbuttonstepquarter', 'pgosce')) . "</option></select></div></div>' +
